...
!(function (exports) {
  ...
  raw.model = function () {
    ...
    model.isValid = function () {
      return (
        dimensions.values().filter(function (d) {
    ...
    model.instruction = function () {
      return dimensions
        ...
        .map(function (d) {
          var v =
            d.required() - d.value.length > 1 ? "dimensions" : "dimension";
          return (
            "<b>" +
            d.title() +
            "</b> 至少需要 " +
            (d.required() - d.value.length) +
            " 更多 " +
            v
          );
        })
  ...
  raw.models.set("tree", function () {
    ...
    var hierarchy = tree
      ...
      .title("层次结构")
      .description(
        "This is a description of the hierarchy that illustrates what the dimension is for and other things."
      )
      .required(1)
      .multiple(true);
    ...
    var color = tree.dimension("color").title("颜色");

    var label = tree.dimension("label").title("标签").multiple(true);
  ...
  raw.models.set("timeSeries", function () {
    ...
    var group = stream.dimension("group").title("组").required(1);
    ...
    var size = stream.dimension("size").title("大小").types(Number);

    var color = stream.dimension("color").title("颜色");
  ...
  raw.models.set("points", function () {
    ...
    var size = points.dimension("size").title("大小").types(Number);

    var color = points.dimension("color").title("颜色");

    var label = points.dimension("label").title("标签").multiple(true);
  ...
  raw.dateFormats = [
    ...
    new RegExp(
      "^[A-z]{3,}(\\,)?[\\-\\_\\.\\/\\s]*([A-z]{3,}(\\,)?[\\-\\_\\.\\/\\s]+)?(0?[1-9]|[12][0-9]|3[01])([A-z]{2})?(\\,)?[\\-\\_\\.\\/\\s]*([0-9]{2,4})" +
        timeFormat +
        "$"
    ),
    new RegExp(
      "^([A-z]{3,}(\\,)?[\\-\\_\\.\\/\\s]*)?((0?[1-9]|[12][0-9]|3[01])([A-z]{2})?(\\,)?[\\-\\_\\.\\/\\s]*)?[A-z]{3,}(\\,)?[\\-\\_\\.\\/\\s]*([0-9]{2,4})" +
        timeFormat +
        "$"
    ),
    //  new RegExp('^([A-z]{3,}(\\,)?\\s+)?(0?[1-9]|[12][0-9]|3[01])([A-z]{2})?[\\-\\_\\.\\/\\s]*(0?[1-9]|1[012])[\\-\\_\\.\\/\\s]*([0-9]{2,4})' + timeFormat + '$'),
    new RegExp(
      "^[A-z]{3,}(\\,)?\\s+([A-z]{3,}(\\,)?\\s+)?(0?[1-9]|[12][0-9]|3[01])?\\s*" +
        timeFormat +
        "\\s+([0-9]{2,4})$"
    ),
  ];
