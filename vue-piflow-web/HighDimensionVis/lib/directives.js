...
angular
        ...
        function update() {
          ...
          function explore(m, el) {
            ...
            for (var c in n) {
                ...
                cel
                  ...
                  .on("click", (d) => {
                    ...
                    d3.select(this).classed(
                      "fa-plus-square-o",
                      d3.select(this.parentNode).classed("json-closed")
                    );
                    d3.select(this).classed(
                      "fa-minus-square-o",
                      !d3.select(this.parentNode).classed("json-closed")
              ...
              cel.append("div").html((d) => {
                var pre = is.array(n) ? "" : "<b>" + c + "</b> : ";
                var text = is.array(n[c]) ? "[" : is.object(n[c]) ? "{" : n[c];
                text +=
                  is.array(n[c]) && !n[c].length
                    ? "]"
                    : is.object(n[c]) && is.empty(n[c])
                    ? "}"
                    : "";
                return pre + text;
              });
  ...
  .directive("chart", function ($rootScope, dataService) {
    // console.log($rootScope, dataService);
    return {
      restrict: "A",
      link: function postLink(scope, element, attrs) {
        function update() {
        ...
        scope.$watch(
          function () {
            if (scope.chart)
              return scope.chart.options().map((d) => {
                return d.value;
              });
          },
          scope.delayUpdate,
  ...
  .directive("colors", function ($rootScope) {
    return {
      ...
      link: function postLink(scope, element, attrs) {
        ...
        function listColors() {
          scope.colors = scope.colorScale.value
            .domain()
            .map((d) => {
              return {
                key: d,
      ...
      link: function postLink(scope, element, attrs) {
        ...
        function over(e, ui) {
          var dimension = ui.item.data().dimension,
            html = isValidType(dimension)
              ? '<i class="fa fa-arrow-circle-down breath-right"></i>Drop here'
              : '<i class="fa fa-times-circle breath-right"></i>Don\'t drop here';
      ...
      link: function postLink(scope, element, attrs) {
        scope.$watch("metadata", (metadata) => {
          ...
          element.find("li").draggable({
            ...
            /*snap: true,
            drag: function (e, ui) {
              ui.offset.left = e.pageX;
              ui.offset.top = e.pageY;
            }*/
          });
        });
    ...
    return {
      ...
      link: function postLink(scope, element, attrs) {
        ...
        function update() {
          ...
          var cells = rows.selectAll("td").data(d3.values).enter().append("td");
          cells.text(String);
        }

        function sort(a, b) {
          if (raw.isNumber(a[sortBy]) && raw.isNumber(b[sortBy]))
  ...
  .directive("coder", function () {
    return {
      restrict: "EA",
      template:
        '<textarea id="source" readonly class="source-area" rows="4" ng-model="svgCode"></textarea>',
      link: function postLink(scope, element, attrs) {
        scope.$on("completeGraph", function () {
          ...
          //   element.indexed = inputindex;
          //   inputindex++;
          //   // console.dir(element);
          //   element.oninput = function (e) {
          //     // console.log(e);
          //     switch (element.indexed) {
          //       case 0:
          //         // valueAsNumber
          //         if (element.value > 1200) {
          //           element.value = 1200;
          //           debugger;
          //         }
          //         break;
  ...
  .directive("downloader", function () {
    return {
      ...
      template:
        ...
        '<button bs-select class="btn btn-default" placeholder="选择类型" ng-model="mode" bs-options="m.label for m in modes">' +
        'Select <span class="caret"></span>' +
        "</button>" +
        '<input class="form-control col-lg-12" placeholder="文件名" type="text" ng-model="filename" id="flieNameinput">' +
      ...
      link: function postLink(scope, element, attrs) {
        ...
        function downloadSvg() {
          ...
          if (isSafari) {
            ...
            saveAs(
              blob,
              (element.find("input").val() ||
                element.find("input").attr("placeholder")) + ".svg"
            );
          }
          let filenameValue =
            document.getElementsByTagName("input")["flieNameinput"];
          // filename = "";
          filenameValue.value = "";
        }
        ...
        function downloadPng() {
          ...
          var html = d3.select(source).node().parentNode.innerHTML;
          ...
          image.onload = function () {
            ...
            if (isSafari) {
              ...
              a.download =
                (scope.filename || element.find("input").attr("placeholder")) +
                ".png";
        ...
        var downloadData = function () {
          ...
          saveAs(
            blob,
            (scope.filename || element.find("input").attr("place" + "holder")) +
            ".json"
          );
        };
