(function () {
  ...
  var chart = raw
    .chart()
    .title("水平线图")
    .thumbnail("images/horizon.png")
    .description("水平线图形通过结合位置和颜色方式，减少垂直空间。")
    .category("时间序列型")
    .model(stream);
  ...
  var bands = chart.number().title("波段").defaultValue(4);
  ...
  chart.draw(function (selection, data) {
    ...
    var y = d3.scaleLinear().range([h * bands(), 0]);
    ...
    function multiple(single) {
      ...
      y.domain([0, maxValue]).range([h, -h * (bands() - 1)]);
