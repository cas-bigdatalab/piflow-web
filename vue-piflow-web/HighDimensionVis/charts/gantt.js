(function () {
  ...
  var group = timechuncks.dimension("groups").title("组");

  // Start Date. It will define the starting point of each bar
  var startDate = timechuncks
    .dimension("startDate")
    .title("开始日期")
    .types(Date)
    .accessor(function (d) {
  ...
  var colorDimension = timechuncks
    .dimension("color")
    .title("颜色")
    .types(String);
  ...
  var chart = raw
    .chart()
    .title("甘特图")
    .thumbnail("images/gantt.png")
    .description(
      "甘特图是一种由亨利·甘特在20世纪10年代发明的图形，用来说明项目进度。甘特图说明了项目的终端元素和总结元素的开始和结束日期。"
    )
    .category("时间块型")
    .model(timechuncks);
  ...
  //left margin
  var marginLeft = chart.number().title("左边距").defaultValue(80);
  ...
  chart.draw(function (selection, data) {
    ...
    items
      .selectAll("rect")
      .data(function (d) {
        // take the data and return the list. it should be changed in the model.
        var p = [];
  ...
  function sortBy(a, b) {
    if (sort() == "Start date (descending)")
      return d3.descending(a.value[0][0].start, b.value[0][0].start);
    if (sort() == "Start date (ascending)")
      return d3.ascending(a.value[0][0].start, b.value[0][0].start);
