(function () {
  ...
  var sizes = model.dimension().title("高度").types(Number);
  ...
  var colorsDimesion = model.dimension().title("颜色").types(String);
  ...
  var chart = raw
    ...
    .title("柱状图")
    .description(
      "条形图或者柱状图是通过矩形条来对数据进行划分，矩形柱高度与柱状图所代表的数值成比例。"
    )
    .thumbnail("images/barChart.png")
    .category("其他型")
    .model(model);
  ...
  var xPadding = chart.number().title("水平填充").defaultValue(0.1);

  // Use or not the same scale across all the bar charts
  var sameScale = chart.checkbox().title("使用相同比例").defaultValue(false);
