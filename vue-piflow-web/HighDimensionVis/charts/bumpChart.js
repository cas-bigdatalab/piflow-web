(function () {
  ...
  var group = stream.dimension().title("组").required(1);
  ...
  var size = stream.dimension().title("大小").types(Number);
  ...
  var chart = raw
    .chart()
    .title("主题河流图")
    .thumbnail("images/bumpChart.png")
    .description(
      "对于连续数据，类似连续的时序数据，可以使用主题河流图来代替堆叠的条形图。"
    )
    .category("时间序列型")
    .model(stream);
  ...
  var padding = chart.number().title("填充").defaultValue(0);

  var normalize = chart.checkbox().title("归一化").defaultValue(false);
  ...
  var showLabels = chart.checkbox().title("显示标签").defaultValue(false);
  ...
  chart.draw(function (selection, data) {
    ...
    var x = d3
      .scaleTime()
      .domain(
        d3.extent(data.values, function (d) {
          return d.date;
        })
      )
      .range([0, +width()]);
