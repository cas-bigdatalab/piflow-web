...
import cn.cnic.provider.flow.FlowGroupPathsMapperProvider;
...
public interface FlowGroupPathsMapper {
    ...
    @Results({
            ...
            @Result(column = "line_inport", property = "inport"),
            @Result(column = "line_port", property = "port"),
            @Result(column = "fk_flow_group_id", property = "flowGroup", many = @Many(select = "cn.cnic.mapper.flow.FlowGroupMapper.getFlowGroupById", fetchType = FetchType.LAZY))
    })
