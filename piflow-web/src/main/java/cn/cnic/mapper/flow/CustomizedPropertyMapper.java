...
import cn.cnic.provider.flow.CustomizedPropertyMapperProvider;
...
public interface CustomizedPropertyMapper {
    ...
    @Results({
            @Result(column = "fk_stops_id", property = "stops", many = @Many(select = "cn.cnic.mapper.flow.StopsMapper.getStopsById", fetchType = FetchType.LAZY))
    })
