...
import cn.cnic.provider.flow.StopsMapperProvider;
...
public interface StopsMapper {
    ...
    @Results({@Result(id = true, column = "id", property = "id"),
            ...
            @Result(property = "dataSource", column = "fk_data_source_id", many = @Many(select = "cn.cnic.mapper.dataSource.DataSourceMapper.getDataSourceById", fetchType = FetchType.LAZY)),
            @Result(column = "id", property = "properties", many = @Many(select = "cn.cnic.mapper.flow.PropertyMapper.getPropertyListByStopsId", fetchType = FetchType.LAZY)),
            @Result(column = "id", property = "customizedPropertyList", many = @Many(select = "cn.cnic.mapper.flow.CustomizedPropertyMapper.getCustomizedPropertyListByStopsId", fetchType = FetchType.LAZY))
    ...
    @Results({@Result(id = true, column = "id", property = "id"),
            @Result(column = "id", property = "properties", many = @Many(select = "cn.cnic.mapper.flow.PropertyMapper.getPropertyListByStopsId", fetchType = FetchType.LAZY)),
            @Result(column = "fk_flow_id", property = "flow", many = @Many(select = "cn.cnic.mapper.flow.FlowMapper.getFlowById", fetchType = FetchType.LAZY))
