	...
	public String runStops(String username, boolean isAdmin, RunStopsVo runStopsVo) throws Exception {
		...
		if (null != testDataIds && testDataIds.length > 0) {
			...
			for (int i = 0; i < testDataIds.length; i++) {
				...
				List<LinkedHashMap<String, Object>> schemaIdAndNameList = testDataDomain
						.getTestDataSchemaIdAndNameListByTestDataId(testDataIds[i]);
				if (null == schemaIdAndNameList || schemaIdAndNameList.size() <= 0) {
					return ReturnMapUtils.setFailedMsgRtnJsonStr("testData error");
				}
				String schema = "dataRow";
				...
				List<LinkedHashMap<String, Object>> schemaValueList = testDataDomain
						.getTestDataSchemaValuesCustomList(isAdmin, username, testDataIds[i], schemaIdAndNameList);
		...
		if (hdfsUrlArray.size() > 0) {
			StopsComponent stopsComponentByBundle = stopsComponentDomain
					.getStopsComponentByBundle("cn.piflow.bundle.csv.CsvParser");
			...
			for (int i = 0; i < hdfsUrlArray.size(); i++) {
				...
				MxCell processMxCell_stops = MxCellUtils.AddMxCellNode(username, processStop.getPageId(),
						stopsComponentByBundle.getName(),
						"/images/" + stopsComponentByBundle.getName() + "_128x128.png");
