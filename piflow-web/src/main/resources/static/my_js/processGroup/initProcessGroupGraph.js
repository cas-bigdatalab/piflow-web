    ...
    if (cell && cell.style && (cell.style).indexOf("image\;") === 0 && processType == "GROUP") {
        ...
        if (processType == "GROUP") {
            ...
            ajaxRequest({
                cache: true,
                type: "POST",
                url: "/page/process/inc/process_info_inc",
        ...
        ajaxRequest({
            cache: true,
            type: "POST",
            url: "/page/process/inc/process_property_inc",
        ...
        if (processType == "GROUP") {
            ...
            ajaxRequest({
                cache: true,
                type: "POST",
                url: "/page/process/inc/process_path_inc",
                data: {processId: processGroupId, pageId: cell.id},
                async: true,
                error: function (request) {
                    return;
            ...
            success: function (data) {
                ...
                if (200 === dataMap.code) {
                    if ('flow' === dataMap.nodeType) {
                        ...
                        window_location_href("/page/process/drawingBoard?drawingBoardType=TASK&parentAccessPath=flowGroupList&load=" + flow_obj.id);
                    } else if ('flowGroup' === dataMap.nodeType) {
                        var flowGroup_obj = dataMap.flowGroupVo;
                        window_location_href("/page/processGroup/drawingBoard?drawingBoardType=GROUP&parentAccessPath=flowGroupList&load=" + flowGroup_obj.id);
                    }
                } else {
